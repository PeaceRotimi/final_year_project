...
import org.powertac.samplebroker.services.RedisBridge;
...
implements MarketManager, Initializable, Activatable
{
  ...
  {
    // convert transaction data to string and send it to Redis
    String data = "MarketTX:" + tx.getMWh()+ ":" + tx.getPrice();

    redisBridge.sendToPython("market_updates",data);
  }
  ...
  {
    String payload = report.getTemperature() + "," + report.getWindSpeed();
    redisBridge.sendToPython("weather_data", payload);
  }
